# Create a 1-of-4 P2SH multisig address from the public keys in the four inputs of this raw tx:
# transaction=""

#!/bin/bash

#The raw Transaction
TRANSACTION=""

#Decode  raw trx
#DEC_RAW=$(bitcoin-cli -regtest decoderawtransaction "$TRANSACTION")

#Extract pub key
#PUBKEYS=($(echo "$DEC_RAW" | jq -r '.vin[].txinwitness[]' | grep -E '^[0-9a-f]{66}$'))

#Check  public keys
#if [ "${#PUBKEYS[@]}" -lt 4 ]; then
 # echo "❌ Error: Could not extract 4 public keys!"
  #exit 1
#fi

#Pick only the first 4 unique pubkeys
#UNIQUE_KEYS=($(printf "%s\n" "${PUBKEYS[@]}" | sort -u | head -n 4))

# Create a 1-of-4 multisig address
#MULTISIG=$(bitcoin-cli -regtest createmultisig 1 "[\"${UNIQUE_KEYS[0]}\",\"${UNIQUE_KEYS[1]}\",\"${UNIQUE_KEYS

# Decode the raw transaction to extract input public keys
DEC_RAW=$(bitcoin-cli -regtest decoderawtransaction "$TRANSACTION")

# Extract public keys from input scripts
PUBKEYS=($(echo "$DEC_RAW" | jq -r '.vin[].txinwitness[1]'))

# Check if we got 4 public keys
if [ "${#PUBKEYS[@]}" -ne 4 ]; then
  echo "❌ Error: Could not extract 4 public keys!"
  exit 1
fi

# Create a 1-of-4 multisig address
MULTISIG=$(bitcoin-cli -regtest createmultisig 1 "[\"${PUBKEYS[0]}\",\"${PUBKEYS[1]}\",\"${PUBKEYS[2]}\",\"${PUBKEYS[3]}\"]")

# Extract the P2SH address
P2SH_ADDRESS=$(echo "$MULTISIG" | jq -r '.address')

# Output the result
echo "$P2SH_ADDRESS"
